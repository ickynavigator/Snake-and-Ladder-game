...
def player_mover(player, num, curr=0):
    ...
    for x in range(curr, location):
        player.penup()
        player.goto(boxLocation[x])
...
def snakeAndLadderGame():
    ...
    player_mover(p1Turtle, 0, playerLastVal[0])

    # setup player 2 turtle
    turtle.addshape("cow.gif")
    p2Turtle = turtle.Turtle()
    p2Turtle.shape("cow.gif")
    player_mover(p2Turtle, 0, playerLastVal[0])

    nextInput = enterInput()
    while (nextInput == 'Y'):
        playerLastVal = [player[0], player[1]]
        ...
        player[i] += diceRoll
        player_mover(p1Turtle if i == 0 else p2Turtle,
                     player[i], playerLastVal[i])
